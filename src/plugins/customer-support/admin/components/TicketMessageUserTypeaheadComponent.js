...
function TicketMessageUserTypeaheadComponent(props) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response?.data) {
          ...
          setInputValue(data.email || data.phone)
          setIsLoading(false)
        }
      } catch (err) {
        console.log(err)
  ...
  const listItems =
    users && users.length
      ? users.map(
          data => <li onClick={() => onClick(data)}><table key={data.id}><tr><td><span>{data.email}</span></td></tr></table></li>, // <li>{element.firstName} {element.lastName}</li>
        )
      : null
...
export default TicketMessageUserTypeaheadComponent
