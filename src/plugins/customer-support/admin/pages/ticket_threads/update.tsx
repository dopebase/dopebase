...
const UpdateTicketThreadView = props => {
  ...
  const initializeModifieableNonFormData = originalData => {
    ...
    /* Insert non modifiable initialization data here */
          if (originalData.is_closed) {
              nonFormData['is_closed'] = originalData.is_closed
          }
          
          if (originalData.is_public) {
              nonFormData['is_public'] = originalData.is_public
          }
  ...
  const saveChanges = async (modifiedData, setSubmitting) => {
    const response = await authPost(
      baseAPIURL + 'ticket_threads/update?id=' + id,
      JSON.stringify({
        ...modifiedData,
        ...modifiedNonFormData,
      }),
...
export default UpdateTicketThreadView
